...
function Home(){
    ...
    return (
        ...
        <div className="relative z-1 p-[10px] flex gap-[10px] bg-[#9e5173] h-[100vh] w-full">
            <div className="flex gap-[10px] mr-[310px] overflow-hidden hover:overflow-x-auto">
            {maintodos?.map((mtodo) => (
                <Task key={mtodo?.id} mId={mtodo.id} isCompleted={mtodo.completed} tit={mtodo.title}/>
            ))}
            </div>
            <div className="absolute z-2 right-[10px] h-[50px] w-[300px] flex gap-[10px] items-center text-white bg-white/30 rounded-lg">
                <img
